...
Links in all Markdown and YAML files are automatically checked using [lychee](https://github.com/lycheeverse/lychee) as part of the ReadMe sync workflow. The check runs on every push to a version branch and will fail the workflow if any broken HTTPS links are found.